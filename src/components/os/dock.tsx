  ...
  return (
    <TooltipProvider delayDuration={100}>
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 select-none max-w-[95vw]">
        <div className="flex items-center gap-2 md:gap-3 px-3 py-2.5 rounded-2xl bg-zinc-900/60 backdrop-blur-2xl border border-white/15 shadow-[0_15px_35px_rgba(0,0,0,0.6)]">
          {DOCK_ITEMS.map((item) => {
            ...
            return (
              <Tooltip key={item.id}>
                  ...
                  >
                    <motion.div
                      ...
                      className={`relative w-11 h-11 md:w-12 md:h-12 rounded-xl flex items-center justify-center transition-all duration-300 ${
                        isActive
                          ? "bg-white/15 border border-cyan-400/50 shadow-[0_0_20px_rgba(6,182,212,0.3)]"
                    ...
                    >
                      ...
                      <Icon
                        className={`w-5 h-5 md:w-6 md:h-6 transition-colors duration-200 ${
                          isActive ? "text-cyan-300" : "text-zinc-300 group-hover:text-white"
                        }`}
                      />
                    </motion.div>
                ...
                <TooltipContent
                  side="top"
                  sideOffset={12}
                  className="bg-zinc-900/90 backdrop-blur-md border border-white/15 text-zinc-100 font-sans text-xs px-2.5 py-1 rounded-lg"
                >
                  {item.label}
                </TooltipContent>
              </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <motion.div
                  ...
                  whileTap={{ scale: 0.95 }}
                  transition={{ type: "spring", stiffness: 400, damping: 25 }}
                  className="w-11 h-11 md:w-12 md:h-12 rounded-xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center hover:bg-cyan-500/20 text-cyan-300 transition-colors"
                >
            ...
            <TooltipContent
              side="top"
              sideOffset={12}
              className="bg-zinc-900/90 backdrop-blur-md border border-cyan-500/30 text-cyan-200 font-sans text-xs px-2.5 py-1 rounded-lg"
            >
              Resume PDF
            </TooltipContent>
          </Tooltip>
